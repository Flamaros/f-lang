...
namespace f
{
    ...
    export inline bool operator ==(const Token& lhs, const Token& rhs)
	{
...
//		return lhs.text == rhs.text;
    ...
    export void    initialize_lexer();
    export void    lex(const fstd::system::Path& path, fstd::memory::Array<Token>& tokens);
    export void    print(fstd::memory::Array<Token>& tokens);


    export inline bool    is_a_basic_type(Keyword keyword) {
        return keyword >= Keyword::VOID && keyword <= Keyword::TYPE;
    }

    export inline bool    is_literal(Token_Type token_type) {
...
import f.lexer.hash_table;
import f.lexer.keyword_hash_table;
...
import fstd.stream.array_stream;
