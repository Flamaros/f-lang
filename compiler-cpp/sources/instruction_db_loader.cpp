...
namespace f
{
	namespace x86_DB
	{
        ...
        static language::string_view x86_keyword_invalid_key;
        ...
        static Keyword_Hash_Table<uint32_t, language::string_view, x86_Keyword, &x86_keyword_invalid_key, x86_Keyword::UNKNOWN>  db_keywords;

        static inline x86_Keyword is_keyword(const language::string_view& text)
        {
            return db_keywords.find(keyword_key(text), text);
        }
...
#define INSERT_KEYWORD(KEY, VALUE) \
        { \
            ...
            db_keywords.insert(keyword_key(str_view), str_view, (x86_Keyword::VALUE)); \
        }

        static void initialize_lexer()
        {
            db_keywords.set_key_matching_function(&language::are_equals);

            INSERT_KEYWORD("ignore", _IGNORE);

            core::log(*globals.logger, core::Log_Level::verbose, "[lexer] keywords hash table: size: %d bytes - nb_used_buckets: %d - nb_collisions: %d\n", db_keywords.compute_used_memory_in_bytes(), db_keywords.nb_used_buckets(), db_keywords.nb_collisions());
        }
