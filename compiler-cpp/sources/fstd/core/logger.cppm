export module fstd.core.logger;
...
namespace fstd
{
	namespace core
	{
		...
		export void set_log_level(Logger& logger, Log_Level level) {
			logger.level = level;
		}

		export Log_Level get_log_level(Logger& logger) {
			return logger.level;
		}

		export void log(Logger& logger, Log_Level level, const char* format, ...) {
		...
		export void log_push_scope(Logger& logger, const char* scope_name) {
		...
		export void log_pop_scope(Logger& logger) {
			logger.indentation--;
		}

		export void flush_logs(Logger& logger) {
