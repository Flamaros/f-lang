export module fstd.language.intrinsic;
...
namespace fstd
{
	namespace intrinsic
	{
		export inline void swap(uint16_t* a, uint16_t* b)
		{
		...
		export inline void swap(uint8_t* a, uint8_t* b)
		{
			// https://c9x.me/x86/html/file_module_x86_id_328.html
			// xchg is certainly not faster as the C implementation
			// xchg does a lock (memory barrier)
		...
		export inline void divide(uint32_t value, uint32_t divisor, uint32_t* quotien, uint32_t* reminder)
		{
...
//#endif
		...
		export inline void divide(uint64_t value, uint64_t divisor, uint64_t* quotien, uint64_t* reminder)
		{
			*quotien = value / divisor;
			*reminder = value % divisor;
		}
