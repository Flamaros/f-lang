...
int main(int ac, char** av)
{
	...
	asm_x64_lexer_file.open(base_path + "/sources/ASM/generated/ASM_x64_lexer.cpp", ios::trunc | ios::binary);
...
R"CODE(    enum class Register : uint8_t // @TODO Can we have more than 256 registers
	...
	stringstream x64_cpp_lexer_ht_insert_instructions;
	stringstream x64_cpp_lexer_ht_insert_registers;

	x64_cpp_lexer_ht_insert_instructions <<
R"CODE(#define HT_INSERT_INSTRUCTIONS \
)CODE";

	x64_cpp_lexer_ht_insert_registers <<
R"CODE(#define HT_INSERT_REGISTERS \
)CODE";
			...
			if (regex_match(read_line, register_desc_match_result, register_desc_regex)) {
				...
				if (regex_match(registers, registers_match_result, registers_regex)) {
					...
					for (int i = first; i <= last; i++) {
						std::string register_enum_lower_case = prefix + to_string(i) + suffix;
						std::string register_enum = to_upper(register_enum_lower_case);
						...
						x64_cpp_lexer_ht_insert_registers << "    " << "HT_INSERT_VALUE(Register, (uint16_t)Keyword::COUNT + (uint16_t)Instruction::COUNT, \"" << register_enum_lower_case << "\", " << register_enum << "); \\" << endl;
					}
				}
				else {
					...
					x64_cpp_lexer_ht_insert_registers << "    " << "HT_INSERT_VALUE(Register, (uint16_t)Keyword::COUNT + (uint16_t)Instruction::COUNT, \"" << registers << "\", " << register_enum << "); \\" << endl;
				}
			}
	...
	if (insns_dat_file.is_open())
	{
		...
		while (getline(insns_dat_file, read_line))
		{
			...
			if (regex_match(read_line, instruction_desc_match_result, instruction_desc_regex)) {
				...
				string	instruction_lower_case = to_lower(instruction);
				...
				if (instruction != previous_instruction_name) {
					if (is_x64_supported) {
						...
						x64_cpp_lexer_ht_insert_instructions << "    " << "HT_INSERT_VALUE(Instruction, Keyword::COUNT, \"" << instruction_lower_case << "\", " << instruction << "); \\" << endl;
...
R"CODE(
	...
	x64_cpp_lexer_ht_insert_instructions << endl;
	x64_cpp_lexer_ht_insert_registers << endl;

	asm_x64_lexer_file << x64_cpp_lexer_ht_insert_instructions.str() << endl;
	asm_x64_lexer_file << x64_cpp_lexer_ht_insert_registers.str() << endl;
